...
func NewClient(server string, ops ...grpc.DialOption) *Client {
	...
	client := proto.NewRfqMmApiClient(conn)
	return &Client{server: server, conn: conn, RfqMmApiClient: client}
}
...
func (s *Server) Serve(ops ...grpc.ServerOption) {
	...
	proto.RegisterRfqMmApiServer(grpcServer, s)
